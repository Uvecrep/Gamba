...
func _spawn_lootbox_pickups(amount: int) -> void:
	...
	for _i in range(amount):
		...
		var drop_direction_sign: float = -1.0 if randf() < 0.5 else 1.0
		var drop_offset := Vector2(
			randf_range(side_drop_min_offset, side_drop_max_offset) * drop_direction_sign,
			randf_range(-side_drop_vertical_jitter, side_drop_vertical_jitter)
		)
		var drop_world_position: Vector2 = _push_drop_outside_player_collect_radius(global_position + drop_offset)

		new_box.set_data(produced_lootbox_id)
		new_box.should_bob = false
		new_box.set_interaction_radius_multiplier(mined_pickup_interaction_radius_multiplier)
		new_box.floating_towards = follow_target
		_set_pickup_position_before_spawn(new_box, pickup_parent, drop_world_position)
...
func _set_pickup_position_before_spawn(pickup: Pickup, _pickup_parent: Node, world_position: Vector2) -> void:
	var pickup_parent_node2d: Node2D = _pickup_parent as Node2D
	if pickup_parent_node2d != null:
		pickup.position = pickup_parent_node2d.to_local(world_position)
		return
